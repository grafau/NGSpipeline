...
#SBATCH -e err/c_read_processing_part2_%A_%a.err
...
merge_bam_files() {
    ...
    if [ ${#merge_files[@]} -gt 1 ]; then
        ...
        samtools merge -o "${HOM}/mapping/${merged_sample}.RG.mapped.sorted.bam" "${merge_files[@]}"
        check_command "Samtools merge ${sample}"
    fi
