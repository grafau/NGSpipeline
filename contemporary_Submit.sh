...
sed "s/ARRAY_SIZE/${ARRAY_SIZE}/" contemporary_part1.sh > $BASE/Array_contemporary_part1.sh
sed "s/ARRAY_SIZE/${ARRAY_SIZE}/" contemporary_part2.sh > $BASE/Array_contemporary_part2.sh

# Submit the job for part 1
PART1_JOB_ID=$(sbatch $BASE/Array_contemporary_part1.sh | awk '{print $4}')

# Submit the job for part 2 with dependency on part 1
PART2_JOB_ID=$(sbatch --dependency=afterok:$PART1_JOB_ID $BASE/Array_contemporary_part2.sh | awk '{print $4}')

# Create and submit PART3 wrapper job
WRAPPER=$BASE/Submit_contemporary_part3_wrapper.sh

cat > "$WRAPPER" <<'EOF'
#!/bin/bash
#SBATCH -J Submit_contemporary_part3
#SBATCH -p short
#SBATCH --mem=1G
#SBATCH -t 00:05:00
...
# Update script
sed "s/ARRAY_SIZE/${LINES}/" "$BASE/contemporary_part3.sh" > "$BASE/Array_contemporary_part3.sh"
chmod +x "$BASE/Array_contemporary_part3.sh"

# Submit the job for Part3
PART3_JOB_ID=$(sbatch "$BASE/Array_contemporary_part3.sh" | awk '{print $4}')
